...
class SarAtrStrategy(CtaTemplate, SubmitTradeMixin):
    """SAR/ATR Strategy"""
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(SarAtrStrategy, self).__init__(
    ...
    def on_bar(self, bar: BarData):
        ...
        sar_array = talib.SAR(am.high, am.low, self.sar_k1, self.sar_k2)
        ...
        if self.pos == 0:
            ...
            if self.atr_value > self.atr_ma:
                ...
                if am.close[-2] < sar_array[-2] and bar.close_price > sar_array[-1]:
                    if not self.reverse:
                        self.buy(bar.close_price, size)
                    else:
                        self.short(bar.close_price, size)
                elif am.close[-2] > sar_array[-2] and bar.close_price < sar_array[-1]:
                    if not self.reverse:
                        self.short(bar.close_price, size)
                    else:
                        self.buy(bar.close_price, size)
